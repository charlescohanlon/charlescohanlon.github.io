...
for $\boldsymbol{U}$ orthonormal, $\boldsymbol{\Lambda}$ diagonal from the eigendecomposition. ZCA is similar to PCA in that they both decorrelate and enforce unit variance. But ZCA appends the inverse rotation to $\boldsymbol{U}^\top$, which can be shown to minimize L2 w.r.t. the pre-transformed data ($\lVert (\boldsymbol{z} - \boldsymbol{\mu}) - \tilde{\boldsymbol{z}} \rVert_2$). [MTGRec](https://arxiv.org/abs/2504.04400) reports that representation whitening improves SID quality.
